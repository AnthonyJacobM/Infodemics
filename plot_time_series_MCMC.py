...
def plot_colorbar(N = 10, CMAP = 'brg', cbar_lab = r'$\delta$', data_row = data_row):
...
def generate_risk_time_series(mc_z = 10, epsilon = 0.01, n0 = 51, tend = 11_000, zvar = 'bad', leg_bool = False):
    ...
    for i, i0 in enumerate(r_bin):
        ...
        sg = np.mean(sg0, axis = 0)
        sb = np.mean(sb0, axis = 0)
        ib = np.mean(ib0, axis = 0)
        ig = 1 - (sg + sb + ib + vac)
        ...
        if zvar == 'inf':
            ...
            ybin = ib
            xbin = sg
        elif zvar == 'phi_delta_e':
            xlab = r'$I - r V$'
            ylab = r'$\phi$'
            xbin = inf - par_ics['risk'] * vac
            ybin = phi
        elif zvar == 'delta_e':
            xlab = r't (Days)'
            ylab = r'$I - r V$'
            ybin = inf - par_ics['risk'] * vac
            xbin = t
        else:
        ...
        if np.max(inf) <= 0.8 and np.min(inf) >= 0 and np.max(bad) <= 1 and np.min(bad) >= 0 and np.max(phi) <= 1.01:
            col = cmap(float(i / n))
            print(i, ylab, ' max: ', np.max(ybin), ' min: ', np.min(ybin))
            if i == 10 or i == 11 or i == 65 or i == 95:
                plt.plot(xbin[1000:], ybin[1000:], 'r-', color = col, lw = 2)
            ...
            if i == 5 or i == 18 or i == 45 or i == 30:
                plt.plot(xbin[1000:], ybin[1000:], 'r-', color=col, lw=5, label=f"r = {rnew}")
...
def plot_risk():
    ...
    :return:
    """
    zvar_bin = ['inf', 'bad', 'good', 'vac', 'phi', 'sb_sg', 'ib_sb', 'v_ib', 'sg', 'sb', 'ib']
    zvar_bin = ['v_ib', 'sb_sg', 'ib_sb', 'ig_ib', 'bad', 'phi_delta_e', 'delta_e']
    zvar_bin = ['vac', 'delta_e', 'phi_delta_e', 'sb_sg']
    for z, z0 in enumerate(zvar_bin):
        generate_risk_time_series(zvar = z0, n0 = 101, mc_z = 10, tend = 10_000)

#plot_risk()
#data_row = np.linspace(0, 2.5, 21)
#plot_colorbar(N = 21, CMAP = 'brg', cbar_lab = r'$r$', data_row = data_row)
...
def generate_education_time_series(mc_z=12, epsilon=0.01, n0=51, tend=10_000, zvar = 'phi', leg_bool = False):
    ...
    for i, i0 in enumerate(edu_bin):
        ...
        if np.max(inf) <= 0.8 and np.min(inf) >= 0 and np.max(bad) <= 1 and np.min(bad) >= 0 and np.max(phi) <= 1.01:
            col = cmap(float(i / n))
            print(i)
            if i == 22 or i == 47 or i == 64 or i == 85:
                plt.plot(xbin[1000:], ybin[1000:], 'r-', color=col, lw=2)
            ...
            if i == 2 or i == 5 or i == 10:
                plt.plot(xbin[1000:], ybin[1000:], 'r-', color=col, lw=4, label=f"$\epsilon = {edu_new}$")
...
def plot_education():
    ...
    :return:
    """
    zvar_bin = ['inf', 'bad', 'good', 'vac', 'phi', 'sg', 'sb', 'ib', 'sb_sg', 'ib_sb', 'v_ib', 'ig_ib']
    zvar_bin = ['delta_e', 'phi_delta_e', 'sb_sg', 'ib_sb', 'v_ib', 'ig_ib']
    for z, z0 in enumerate(zvar_bin):
        generate_education_time_series(zvar = z0, n0 = 101, mc_z = 10, tend = 11_000)

"""plot_education()
data_row = np.linspace(0, 0.4, 10)
plot_colorbar(N = 10, CMAP = 'brg', cbar_lab = r'$\epsilon$', data_row = data_row)"""

#quit()

def generate_misinformation_time_series(mc_z=12, epsilon=0.01, n0=51, tend=11_000, zvar='phi', leg_bool = False, short_bool = False):
    """
    function to generate a time series in education
    :param mc_z: monte carlo in z
    :param epsilon: weight of the noise
    ...
    for i, i0 in enumerate(edu_bin):
        ...
        if np.max(inf) <= 0.8 and np.min(inf) >= 0 and np.max(bad) <= 1 and np.min(bad) >= 0 and np.max(phi) <= 1.01:
            col = cmap(float(i / n))
            print(i)
            if i == 0 or i == 1 or i == 10 or i == 25 or i == 35 or i == 45:
                plt.plot(xbin[1000:], ybin[1000:], 'r-', color=col, lw=2)
            ...
            if short_bool == True:
                if i == 1 or i == 10 or i == 18:
                    plt.plot(xbin[1000:], ybin[1000:], 'r-', color=col, lw=4, label=f"$\mu = {mu_new}$")
            else:
                plt.plot(xbin[1000:], ybin[1000:], 'r-', color=col, lw=4, label=f"$\mu = {mu_new}$")
...
plot_colorbar(N = 10, CMAP = 'brg', cbar_lab = r'$\mu$', data_row = data_row)
...
def plot_misnformation():
    ...
    zvar_bin = ['inf', 'bad', 'good', 'vac', 'phi', 'sg', 'sb', 'ib', 'sb_sg', 'ib_sb', 'v_ib', 'ig_ib']
    zvar_bin = ['delta_e', 'phi_delta_e', 'sb_sg', 'ig_ib', 'ib_sb', 'v_ib']
    for z, z0 in enumerate(zvar_bin):
        generate_misinformation_time_series(zvar=z0, n0=21, mc_z=10, tend=11_000, leg_bool = False, short_bool = False)
...
def generate_protection_time_series(mc_z=12, epsilon=0.01, n0=51, tend=11_000, zvar='phi', leg_bool = False, short_bool = False):
    """
    function to generate a time series in education
    :param mc_z: monte carlo in z
    :param epsilon: weight of the noise
    ...
    for i, i0 in enumerate(del_bin):
        ...
        if np.max(inf) <= 0.8 and np.min(inf) >= 0 and np.max(bad) <= 1 and np.min(bad) >= 0 and np.max(phi) <= 1.01:
            ...
            if short_bool == False:
                ...
                if i == 1 or i == 11 or i == 21 or i == 31 or i == 41 or i == 48 or i == 50:
                    plt.plot(xbin[1000:], ybin[1000:], 'r-', color=col, lw=2)
            ...
            if i == 1 or i == 10 or i == 18:
                plt.plot(xbin[1000:], ybin[1000:], 'r-', color=col, lw=4, label=f"$\delta = {del_new}$")
...
def plot_protection():
    ...
    zvar_bin = ['inf', 'bad', 'good', 'vac', 'phi', 'sg', 'sb', 'ib', 'sb_sg', 'ib_sb', 'v_ib', 'ig_ib']
    zvar_bin = ['vac', 'delta_e', 'phi_delta_e', 'sb_sg', 'ib_sb', 'v_ib', 'ig_ib']
    for z, z0 in enumerate(zvar_bin):
        generate_protection_time_series(zvar=z0, n0=51, mc_z=10, tend=11_000, leg_bool = False, short_bool = True)



data_row = np.linspace(0.5, 1.0, 10)
plot_colorbar(N = 10, CMAP = 'brg', cbar_lab = r'$\delta$', data_row = data_row)
