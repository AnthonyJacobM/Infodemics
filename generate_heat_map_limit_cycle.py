...
def generate_heat_map_limit_cycle(PC, curve = 'EQmisinformation', special_point = 'H1', xpar = 'misinformation', ypar = 'education', par_dict = eq2_h2_par_dict, ss_dict = {}, max_n_points = 35, curve_type = 'H-C1', name_curve = 'HO1', n_bin = 10, tend = 4_000, eps = 0.01):
    """
    ...
    for x, x0 in enumerate(par_x_bin):
        for y, y0 in enumerate(par_y_bin):
            ...
            if len(peak_inf_idx) > 0:
                inf_peak = np.max(inf[peak_inf_idx])
            else:
                inf_peak = inf[-1]
            if len(peak_bad_idx) > 0:
                bad_peak = np.max(bad[peak_bad_idx])
            else:
                bad_peak = bad[-1]
            if len(peak_vac_idx) > 0:
                vac_peak = np.max(v[peak_vac_idx])
            else:
                vac_peak = v[-1]
    ...
    # load the data!
    #inf_data = np.loadtxt('')
    #save_name = '\heatmap.jpg', data = array, xlabel = '$r', ylabel = '$\tilde{\chi}_{bg}$', col_labels = row_array, row_labels = col_array, cmap = \
    #cmap_bin['hot']
