...
def plot_infection_good_bifurcation(par_dict=eq1_h1_par_dict, ics_dict=eq1_h1_ss, special_point='H2',
    ...
    PC_infection_good = generate_infection_good_bifurcation(ode, ics_dict=ics_dict, par_dict=par_dict, tend=300)

    # get the data
    if special_point != 'BP1':
        par_dict, ss_dict, data = get_data(PC_infection_good, curve='EQinfection_good', special_point=special_point,
                                           par_dict=par_dict,
                                           par='infection_good')

    # generate a pointset and plot the time around the bifurcation!
    pts = plot_time_perturbed_steady_states(PAR_dict=par_dict, ss_dict=ss_dict, tend=tend, par='infection_good',
                                            random_bool=True,
                                            eps=eps0)
    ...
    return PC_infection_good, par_dict, ss_dict, data
